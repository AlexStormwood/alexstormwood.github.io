...
async function galleryImageShortcode(src, alt) {
  ...
  let lightboxImageWidth = LANDSCAPE_LIGHTBOX_IMAGE_WIDTH;
    ...
    return `
        <a href="${genMetadata.jpeg[genMetadata.jpeg.length - 1].url}" 
        data-pswp-width="${genMetadata.jpeg[genMetadata.jpeg.length - 1].width}" 
        data-pswp-height="${genMetadata.jpeg[genMetadata.jpeg.length - 1].height}" 
        target="_blank">
            <img src="${genMetadata.jpeg[genMetadata.jpeg.length - 1].url}" alt="${alt}" />
        </a>
    `.replace(/(\r\n|\n|\r)/gm, "");;
}
